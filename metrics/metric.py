...
def calculate_metrics(**kwargs):
    ...
    if model_cfg.model_name == 'ncsnpp':
        ...
        unet = create_model(unet_cfg)
    elif model_cfg.model_name == 'ncsnpp10':
        unet_cfg = model_cfg.ncsnpp
        from unet_plus.ncsnpp10 import create_model
        unet = create_model(unet_cfg)
    ...
    save_fname = f'metric_results_{config.model.sampling_timesteps}_{cur_time}.json'
    with open(os.path.join(config.eval.workdir, save_fname), 'w') as f:
