...
def main(args):
    ...
    nerf_cfg = model_cfg.nerf
    nerf_kwargs = {'cfg': nerf_cfg}
    nerf_kwargs.update(nerf_cfg)
    nerf = construct_class_by_name(**nerf_kwargs)
            ...
            for j in range(len(rgbss)):
                ...
                for rgb_i in range(len(rgbs)):
                    tv.utils.save_image(rgbs[rgb_i],
                        str(trainer.test_folder / f'ckpt-{train_cfg.resume_milestone}-testbefore-{j}-image-{rgb_i}.png'))
                    tv.utils.save_image(parts[rgb_i],
                                        str(trainer.test_folder / f'ckpt-{train_cfg.resume_milestone}-testbefore-{j}-part-{rgb_i}.png'))
...
class Trainer(object):
    def __init__(
        ...
        self.results_folder = Path(results_folder)
        self.test_folder = Path(cfg.trainer.test_folder)
        if self.accelerator.is_main_process:
    ...
    def train(self):
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if self.step >= self.cfg.model.joint_step:
                    self.opt_v.step()
