...
class Trainer(object):
    ...
    def load(self, milestone):
        ...
        for k1 in list(model_dict.keys()):
            if 'rgbnet' in k1:
                data['model'][k1] = model_dict[k1]
            ...
            while self.step < self.train_num_steps:
                ...
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        if isinstance(self.model, nn.parallel.DistributedDataParallel):
                            loss, log_dict = self.model.module.training_step(batch, ga_ind, self.step, accelerator=accelerator)
                        else:
                            loss, log_dict = self.model.training_step(batch, ga_ind, self.step, accelerator=accelerator)
