# Copyright (c) 2022, GuHuangAI. All rights reserved.
#
# This model is the 2D Unet used for 3D input.
...
class FlatConv(nn.Module):
    def __init__(self, in_dim=4, voxel_shape=64, embed_dim=32):
        super(FlatConv, self).__init__()
        init = dict(init_mode='kaiming_uniform', init_weight=np.sqrt(1 / 3), init_bias=np.sqrt(1 / 3))
        self.conv1 = Conv2d(in_channels=in_dim*voxel_shape, out_channels=embed_dim, kernel=3, **init)
        self.conv2 = Conv2d(in_channels=in_dim*voxel_shape, out_channels=embed_dim, kernel=3, **init)
        self.conv3 = Conv2d(in_channels=in_dim*voxel_shape, out_channels=embed_dim, kernel=3, **init)

    def forward(self, x):
        # x: b, c, h, w, d
        b, c, h, w, d = x.shape
        x1 = rearrange(x, 'b c h w d -> b (c h) w d')
        x2 = rearrange(x, 'b c h w d -> b (c w) h d')
        x3 = rearrange(x, 'b c h w d -> b (c d) h w')
        x1 = self.conv1(x1)
        x2 = self.conv2(x2)
        x3 = self.conv3(x3)
...
class InvFlatConv(nn.Module):
    def __init__(self, in_dim=128, voxel_shape=64, embed_dim=32, out_dim=4):
        super(InvFlatConv, self).__init__()
        init = dict(init_mode='kaiming_uniform', init_weight=np.sqrt(1 / 3), init_bias=np.sqrt(1 / 3))
        self.conv1 = Conv2d(in_channels=in_dim, out_channels=out_dim * voxel_shape, kernel=3, **init)
        self.conv2 = Conv2d(in_channels=in_dim, out_channels=out_dim * voxel_shape, kernel=3, **init)
        self.conv3 = Conv2d(in_channels=in_dim, out_channels=out_dim * voxel_shape, kernel=3, **init)
        self.out_conv = nn.Conv3d(3 * out_dim, out_dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x2 = self.conv2(x)
        x3 = self.conv3(x)
        assert x1.shape[-1] == x1.shape[-2]
        vs = x1.shape[-1]
        x1 = rearrange(x1, 'b (c h) w d -> b c h w d', h=vs)
        x2 = rearrange(x2, 'b (c h) w d -> b c w h d', h=vs)
        x3 = rearrange(x3, 'b (c h) w d -> b c d w h', h=vs)
        x = torch.cat([x1, x2, x3], dim=1)
        x = self.out_conv(x)
        return x
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        self.input_flatconv = FlatConv(in_dim=in_channels, voxel_shape=img_resolution, embed_dim=embed_dim)
        ...
        # self.out_conv = Conv2d(in_channels=cout1, out_channels=out_channels, kernel=3, **init_zero)
        self.out_conv = InvFlatConv(in_dim=cout1, voxel_shape=img_resolution, out_dim=out_channels)
        ...
        # self.out_conv2 = Conv2d(in_channels=cout2, out_channels=out_channels, kernel=3, **init_zero)
        self.out_conv2 = InvFlatConv(in_dim=cout2, voxel_shape=img_resolution, out_dim=out_channels)

    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        # Mapping.
        x = self.input_flatconv(x)
        emb = self.map_noise(noise_labels)
        if self.map_augment is not None and augment_labels is not None:
            emb = emb + self.map_augment(augment_labels)
        emb = silu(self.map_layer0(emb))
        ...
        for block in self.dec2.values():
            ...
            x2 = block(x2, emb)
        x2 = self.out_conv(silu(self.out_norm(x2)))
        return x1, x2
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels,
                                           embed_dim=embed_dim, out_channels=img_channels, label_dim=label_dim, **model_kwargs)
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        assert F_x.dtype == dtype
        D_x = c_skip1 * x + c_out1 * F_x.to(torch.float32)
        D_y = c_skip2 * x + c_out2 * F_y.to(torch.float32)
        return D_x, D_y
