...
class NeRF(nn.Module):
    ...
    def render_loss(self, field, render_kwargs, **kwargs):
        ...
        for dens, fea, HW, Ks, near, far, i_train, i_val, i_test, poses, images, masks, lw, idx, pf, tf in \
                zip(densities, features, HWs, Kss, nears, fars, i_trains, i_vals, i_tests, posess, imagess, maskss, loss_weights, idxs, part_fea, text_emb):
            device = dens.device
            ...
            for iter in range(inner_iter):
                ...
                # t_n = time.time()
                # print(t_n - t_cur)
                render_result = self.render_train(dens, fea, rays_o, rays_d, viewdirs, idx, pf, tf,
                                                **render_kwarg_train)
                # t_n2 = time.time()
                # print(t_n2 - t_n)
    ...
    def get_part_index(self, dens, fea, ray_pts, part_fea, chunk_size=8192):
        ...
        fea = self.text_att(fea, part_fea)
