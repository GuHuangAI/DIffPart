...
# without part-aware shape decoder
...
class NeRF(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.partnet = nn.Sequential(
            nn.Linear(dim0, rgbnet_width), nn.ReLU(inplace=True),
            *[
                nn.Sequential(nn.Linear(rgbnet_width, rgbnet_width), nn.ReLU(inplace=True))
                for _ in range(rgbnet_depth - 2)
            ],
            nn.Linear(rgbnet_width, 5),
        )
    ...
    def render_train(self, dens, fea, rays_o, rays_d, viewdirs, part_shape_fea, part_text_fea, **render_kwargs):
        ...
        if ray_id.shape[0] > 0:
            if self.rgbnet is None:
                ...
                index_pred = self.partnet(k0_view)
        else:
            rgb_logit = torch.zeros(k0.shape[0], 3, device=k0.device) - 100
            rgb = torch.sigmoid(rgb_logit)
            index_pred = torch.zeros(k0.shape[0], 5, device=k0.device)
        # time5 = time.time()
        # print(time5 - time4)
        # Ray marching
