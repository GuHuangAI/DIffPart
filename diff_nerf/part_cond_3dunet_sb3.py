...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.cond_mlps.append(nn.Linear(part_dim*2, dims[0]))
        self.cond_mlps.append(nn.Linear(part_dim*2, dims[1]))
        self.cond_mlps.append(nn.Linear(part_dim*2, dims[2]))
        self.cond_mlps.append(nn.Linear(part_dim*2, dims[3]))

        # image embedding
        self.projects = nn.ModuleList([])
        self.projects.append(nn.Linear(512, dims[0]))
    ...
    def forward(self, x, time, cond1=None, cond2=None, image=None, *args, **kwargs):
        ...
        for i, ((block1, block2, attn, downsample), relation_layer) in \
            ...
            h.append(x)
            # h2.append(x.clone())
            x = relation_layer(x, torch.cat([part_embs[i], img_embs[i].unsqueeze(1)], dim=1))
            x = block2(x, t)
            x = attn(x)
            h.append(x)
            # h2.append(x.clone())
        ...
        for (block1, block2, attn, upsample), relation_layer in zip(self.ups, self.relations_up):
            ...
            x = block1(x, t)
            x = relation_layer(x, torch.cat([part_embs.pop(), img_embs.pop().unsqueeze(1)], dim=1))
            x = torch.cat((x, h.pop()), dim = 1)
            x = block2(x, t)
            x = attn(x)
